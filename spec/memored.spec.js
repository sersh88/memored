...
describe('Memored test suite', function () {
    ...
    if (cluster.isMaster) {
        ...
        describe('Memored - store', function () {
            ...
            it('Should store a value with promise', async () => {
                var user189 = _createUser();
                var ret = await memored.store('user189', user189);
                expect(ret).to.equal(undefined);
                var stored = await memored.read('user189');
                expect(stored).to.deep.equal(user189);
            });
            ...
            it('Should atomic read current value and set new value in the cache', function (done) {
                var user3 = _createUser();
                memored.readstore('user3', user3, 100, function (err, value) {
                    expect(value).to.be.undefined();
                    memored.read('user3', function (err, value) {
                        expect(value).to.deep.equal(user3);
                        done();
                    });
                });
            });

            it('Should atomic increment value', function (done) {
                var initial_value = 10, increment_by = 5;
                async.series({
                    storeInitial: function (next) {
                        memored.store('inc_key', initial_value, next);
                    },
                    increment: function (next) {
                        memored.increment('inc_key', increment_by, function (err) {
                            expect(err).to.equal(null);
                            next();
                        });
                    },
                    readIncrementedValue: function (next) {
                        memored.read('inc_key', function (err, value) {
                            expect(value).to.equal(initial_value + increment_by);
                            next();
                        });
                    }
                }, done)
